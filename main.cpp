...
void DATA_T(Arguments *in, Reply *out);
void DET_Thread(Arguments *in, Reply *out);
RPCFunction DATA(&DATA_T, "DATA_T");
...
bool is_end = false;
int is_featured[10] = {0};
int gesture_ID = 0;
int gesture_class[10] = {0};
...
int GUI_mode() {
  ...
  while (i == 1) {
    ...
    if (gesture_index < label_num) {
      ...
      //is_end = true;
      gesture_class[gesture_ID] = gesture_index;
      printf("\rgesture ID: %d, gesture class: %d\r\n", gesture_ID, gesture_index);
      is_end = true;
      return 1;
    }
...
int DET_mode(){
  ...
  is_end = false;
  if(maximum >= target_angle) is_featured[gesture_ID] = 1;
  else is_featured[gesture_ID] = 1;
    sprintf(buffer_send[gesture_ID],"gesture ID: %d, max angle: %.1f, is_featured: %d\r\n", gesture_ID, maximum, is_featured[gesture_ID]);
    printf("%s\r\n",buffer_send[gesture_ID]);
  }
  return 1;
}
void DET_Thread(Arguments *in, Reply *out){
